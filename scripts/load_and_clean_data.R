...
ds_2023 <- fread("dataset/NYPD.csv") %>%
  dplyr::select(-ARREST_KEY,-PD_CD,-KY_CD,-LAW_CODE,-JURISDICTION_CODE,-X_COORD_CD, -Y_COORD_CD, -Latitude, -Longitude) 
...
ds_weather <- fread("dataset/weather.csv") %>% 
  dplyr::select(datetime, temp, windspeed, conditions,cloudcover)
