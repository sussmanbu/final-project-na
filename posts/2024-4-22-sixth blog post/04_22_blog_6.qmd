...
ds |> dplyr::filter(OFNS_DESC == 'ASSAULT 3 & RELATED OFFENSES',
                    ARREST_DATE >= as.Date("2023-9-01") & ARREST_DATE <=  as.Date("2023-11-30"))|>
  group_by(Longitude, Latitude) |>
  count(name = 'n') |> 
  mutate(G = cut(n, breaks = c(0, 1, 2, 3, 1523),
                 labels = c('1', '2', '3', '>3'), 
...
plot_2023asf<- ggmap(a) +
  geom_point(data = df_23asf, aes(x = Longitude, y = Latitude, color = G),
             alpha = 0.5, size = 0.5) +
  labs(x = 'Longitude', y = 'Latitude', color = '', title = 'ASSAULT 3 2023f') +
  scale_color_manual(values = c('seagreen', 'blue', 'orange', 'tomato')) +
  theme(legend.position = 'top')
...
We found that when season changes from summer to fall, the crime density gradually decreases.
