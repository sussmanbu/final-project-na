...
Since we know that Level 3 Assault is the most common cases in the New
York city, so we decide to locate our target on the Demographic factor
and Environmental factor that would effect the incidents of Level 3
Assault. Through this analysis, We could uncover correlations that
inform targeted interventions, enhance policy-making, and improve
resource allocation. By identifying specific risk factors related to
demographics and environmental conditions, the analysis aims to guide
more effective prevention strategies, optimize urban planning, and
...
#Borough
```{r echo=FALSE, message=FALSE, warning= FALSE}
assault_3_over_time_monthly <- ds %>%
  filter(grepl("ASSAULT 3 & RELATED OFFENSES", OFNS_DESC)) %>%
  mutate(ARREST_DATE = ymd(ARREST_DATE), Month = floor_date(ARREST_DATE, "month")) %>%
  group_by(Month, ARREST_BORO) %>%
  summarize(Incident_Count = n(), .groups = 'drop') %>%
  arrange(Month) %>%
  # Rename boroughs by changing the factor levels
  mutate(ARREST_BORO = factor(ARREST_BORO, 
                              levels = c("B", "K", "M", "Q", "S"),
                              labels = c("Bronx", "Brooklyn", "Manhattan", "Queens", "Staten Island")))

# Plot the number of Assault 3 incidents over time by borough on a monthly basis
ggplot(assault_3_over_time_monthly, aes(x = Month, y = Incident_Count, color = ARREST_BORO)) +
  geom_line() +
  theme_minimal() +
  labs(x = 'Month', y = 'Number of Assault 3 Incidents', 
       title = 'Monthly Assault 3 Incidents by Borough',
       color = 'Borough') +  # This changes the legend title
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
