...
"ASSAULT 3". This helps police understand under what
...
kable(conf_matrix_df, format = "html", caption = "Confusion Matrix")

kable(summary_stats, format = "html", digits = 4, caption = "Model Performance Metrics")
...
gt(high_confidence_cases) %>%
  tab_header(
    ...
    subtitle = "Displaying cases with prediction probability above 70%"
  ) %>%
  cols_label(
  ...
  fmt_number(
    columns = vars(temp, windspeed, cloudcover),
    decimals = 1  # Adjust the number of decimals for numerical columns
  ) %>%
  tab_options(
    table.width = pct(100),  
  ...
  tab_footnote(
    footnote = "Data sourced from validated records with high predictive confidence.",
...
kable(conf_matrix_tree_df, format = "html", caption = "Confusion Matrix")

kable(summary_stats_tree, format = "html", digits = 4, caption = "Model Performance Metrics")
```
